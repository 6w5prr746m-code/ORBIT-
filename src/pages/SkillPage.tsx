  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-8 sm:px-10">
      ...
      <p className="mt-3 max-w-xl text-[15px] text-graphite">
        People with {skill.name} expertise across your organization.
      </p>

      <div className="mt-10 grid grid-cols-1 gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-graphite-soft">Top experts</h2>
          <div className="flex flex-col gap-2">
            {holders.slice(0, 5).map((h) => (
              <div key={h.person.id} className="flex items-center justify-between rounded-[var(--radius-control)] border border-border bg-canvas-raised px-4 py-3">
                ...
                <Badge variant={h.level === 'expert' ? 'accent' : 'default'}>{h.level}</Badge>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-8">
          {topTeams.length > 0 && (
            <div>
              <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-graphite-soft">Teams</h2>
              <div className="flex flex-col gap-2">
          ...
          {topLocations.length > 0 && (
            <div>
              <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-graphite-soft">Locations</h2>
              <div className="flex flex-col gap-2">
      ...
      {relatedSkills.length > 0 && (
        <section className="mt-10">
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-graphite-soft">Related skills</h2>
      ...
      <section className="mt-10">
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wide text-graphite-soft">Find someone</h2>
        <SearchBar placeholder="Filter people with this skill…" value={query} onChange={(e) => setQuery(e.target.value)} className="mb-4 max-w-xl" />
