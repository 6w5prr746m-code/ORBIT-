  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-graphite-soft">{peopleCount} people</p>
      </div>
      <p className="line-clamp-2 text-xs text-graphite-soft">{reason}</p>
    </Link>
  )
...
export function DiscoverPage() {
  ...
  return (
    <div>
      <PageHeader title="Discover" description="Discover your organization." />

      <div className="flex flex-col gap-10 px-6 py-8 sm:px-10">
        {rows.map((row) => (
          <section key={row.id}>
            <div className="mb-3">
              <h2 className="text-lg font-semibold tracking-tight text-ink">{row.title}</h2>
              <p className="text-sm text-graphite">{row.description}</p>
            </div>
